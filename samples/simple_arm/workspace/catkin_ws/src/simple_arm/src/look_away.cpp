// A good example of clients of nodes that invoke services , and also how a node can subscribe to multiple topics

//To see a ROS subscriber and client in action, you'll write a node called look_away. The look_away node will subscribe to the /rgb_camera/image_raw topic, which has image data from the camera mounted on the end of the robotic arm. Whenever the camera is pointed towards an uninteresting image - in this case, an image with uniform color - the callback function will request a safe_move service to safely move the arm to something more interesting. 

#include "ros/ros.h"
#include "simple_arm/GoToPosition.h"
#include <sensor_msgs/JointState.h>  // to read the arms joints' positions
...
int main(int argc, char** argv)
{
    ...
    ros::NodeHandle n; // the node for lookaway
    ...
    // note that when sub1 recieves a message on /simple/arm/joint_states topic, it calls joint_states_callback.
    // This callback checks what is our tolerance, checks if the arms have stopped moving, and updates our state of the joints' last position.
    ...
    // note that sub2, whenever it gets a camera feed, it checks if we are looking a boring image, checks if we are moving (using sub1's data)
    // and accordingly shifts the robot arms to where camera can get us meaningful feed ..
