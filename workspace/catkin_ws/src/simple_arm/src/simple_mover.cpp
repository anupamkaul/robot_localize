...
int main(int argc, char** argv)
{
    ...
    // Create a publisher that can publish std_msgs::Float64 message on the topic /simple_arm/joint_1_position_controller/command
    ros::Publisher joint1_pub = n.advertise<std_msgs::Float64>("/simple_arm/joint_1_position_controller/command", 10);

    // Create a publisher that can publish std_msgs::Float64 message on the topic /simple_arm/joint_2_position_controller/command
    ...
    while (ros::ok()) {
       ...
       // Sleep for time remaining till 10 Hz is reached
       loop_rate.sleep();
