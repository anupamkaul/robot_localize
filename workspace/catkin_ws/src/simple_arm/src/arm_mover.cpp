...
// Arm mover contains the ROS Service Safe-Move that allows independent
// ROS nodes to call this service to execute safe moves of the robotic arm.
...
{
    // Define clamped joint angles and assign them ot the requested ones
    ...
    //Get joints min and max parameters (from the parameter server?)
...
// Node calls services, that gets interpreted by arm_mover, intermediate clamping action is done, and then arm_mover
// publishes this to simple_mover's topic, so simple_mover executes the robotic arms as before
...
bool handle_safe_move_request(simple_arm::GoToPosition::Request& req, simple_arm::GoToPosition::Response& res)
{
    ...
    // Wait 3 seconds for arm to settle (blocking call !! bad ..)
